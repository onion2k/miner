...
export const BOT_SPEC: DozerSpec = {
  maxSpeed: 7.5,
  accel: 12,
  turnRate: 1.9,
  bladeWidth: 7,
...
export class Bot {
  ...
  private giveWay(want: Drive, traffic: Traffic, nav: Nav): Drive {
    ...
    let yieldTo: Mover | null = null,
      yieldAt = Infinity;
    let blocker: Mover | null = null,
      blockerAt = Infinity;
    ...
    for (const o of movers) {
      ...
      const vx = myC * mySpeed - Math.cos(o.yaw) * oSpeed,
        vy = myS * mySpeed - Math.sin(o.yaw) * oSpeed;
      ...
      const first =
        !o.player && (o.priority > mine || (o.priority === mine && mine === 0 && o.rank < this.dozer.owner));
      if (first && oSpeed !== 0 && d < yieldAt) {
        yieldTo = { ...o, speed: oSpeed };
        yieldAt = d;
        continue;
      }
      // one that has to make way for this one is left to do it; only one that does not is steered round or followed
      const makesWay =
        !o.player && (mine > o.priority || (o.priority === mine && mine === 0 && this.dozer.owner < o.rank));
      const along = myC * -dx + myS * -dy;
      if (!makesWay && want.throttle !== 0 && along * Math.sign(want.throttle) > 0 && d < blockerAt) {
        blocker = o;
        blockerAt = d;
      }
    }
    if (yieldTo) {
      ...
      const c = Math.cos(yieldTo.yaw) * goes,
      ...
      return {
        ...
        steer: ahead && alongMine >= 0 ? this.toward(ahead[0], ahead[1], 1).steer * 0.5 : 0,
      };
    }
    if (blocker) {
      this.headway();
...
export class Fountain {
  ...
  update(
    dt: number,
    spawn: (kind: number, x: number, y: number, z: number, vx: number, vy: number, vz: number) => boolean,
    crack: () => void,
  ) {
    this.timer -= dt;
    switch (this.state) {
      ...
      case 'spray': {
        ...
        while (this.spill >= 1) {
          ...
          if (!spawn(kind, this.x, this.y, 0.8, Math.cos(a) * spread, Math.sin(a) * spread, 18 + Math.random() * 10)) {
            this.timer = 0;
            break;
          }
        }
