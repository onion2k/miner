...
const GEM_ALBEDO: [number, number, number][] = [
  [0, 0, 0],
  [1.0, 0.06, 0.12],
  [0.08, 0.95, 0.35],
  [0.12, 0.35, 1.0],
...
async function main() {
  ...
  for (let i = 0; i < economy.save.drones; i++)
    bots.push(new Bot(world.solid, i + 1, HOLE.x + 14 + i * 6, HOLE.y + 10));
  /** The belts that run: those bought, for rooms not sealed. */
  const running = () =>
    AREAS.map((_, a) => a).filter((a) => AREAS[a].belt && economy.save.belts[a] && !economy.sealed(a));
  ...
  function knockLamps() {
    ...
    const bx = dozer.x + c * BLADE_AT,
      by = dozer.y + sn * BLADE_AT;
    cave.lamps.forEach((l, k) => {
      if (economy.save.lampsBroken.includes(k)) return;
      if (Math.hypot(l.x - dozer.x, l.y - dozer.y) > LAMP_KNOCK && Math.hypot(l.x - bx, l.y - by) > LAMP_KNOCK - 0.8)
        return;
      const lit = lampOn(k);
      economy.breakLamp(k);
      sound.shatter();
  ...
  function buildStatic() {
    ...
    if (!terrain || terrain.key !== revealedKey)
      terrain = { key: revealedKey, ...buildTerrain(cave, economy.save.secrets) };
    ...
    const stoneGroups: GameGroup[] = meshes.stones.map((mesh, shape) => {
      ...
      const m = new Float32Array(Math.max(1, mine.length) * 16),
        mat = new Float32Array(Math.max(1, mine.length) * MATERIAL_STRIDE);
      mine.forEach((st, i) => {
    ...
    const spireM = new Float32Array(Math.max(1, spires.length) * 16),
      spireMat = new Float32Array(Math.max(1, spires.length) * MATERIAL_STRIDE);
    spires.forEach((sp, i) => {
    ...
    gates.forEach(([x, y, a], i) =>
      placePart(gateM, i, x, y, 0, hash(x, y, a) * 0.5 - 0.25, 0, 0, 0, 0, 0, 1, 1, 2.6 + hash(x, y) * 1.2),
    );
    ...
    WALLS.forEach((wall, w) => {
      ...
      const bricks = layBricks(w),
        gold = new Map(treasureBricks(w).map((t) => [t.brick, t.kind]));
      bricks.forEach((b, i) => {
        ...
        const colour =
          kind === BAR
            ? [1.0, 0.72, 0.18, 0.2]
            : [...WALL_COLOUR[wall.grade].slice(0, 3).map((c) => c * shade), WALL_COLOUR[wall.grade][3]];
        const x = b.x + j(1) * hurt * 0.9,
          y = b.y + j(2) * hurt * 0.9;
        standing.push({
          x,
          y,
    ...
    const brickM = new Float32Array(Math.max(1, standing.length) * 16),
      brickMat = new Float32Array(Math.max(1, standing.length) * MATERIAL_STRIDE);
    ...
    const studM = new Float32Array(Math.max(1, studs.length) * 16),
      studMat = new Float32Array(Math.max(1, studs.length) * MATERIAL_STRIDE);
    studs.forEach((g, i) => {
    ...
    const postM = new Float32Array(Math.max(1, cave.lamps.length) * 16),
      headM = new Float32Array(Math.max(1, cave.lamps.length) * 16);
    ...
    cave.lamps.forEach((l, k) => {
      ...
      const hx = l.x + Math.sin(yaw) * Math.sin(pitch) * reach,
        hy = l.y - Math.cos(yaw) * Math.sin(pitch) * reach,
        hz = (down ? 0.6 : 0) + Math.cos(pitch) * reach;
  ...
  const hullM = new Float32Array(16),
    darkM = new Float32Array(16),
    bladeM = new Float32Array(16);
  const treadM = new Float32Array((1 + BOT_CAPACITY) * TREAD_BARS * 2 * 16);
  const botHullM = new Float32Array(BOT_CAPACITY * 16),
    botDarkM = new Float32Array(BOT_CAPACITY * 16),
    botBladeM = new Float32Array(BOT_CAPACITY * 16);
  const stripeM = new Float32Array(STRIPE_CAPACITY * 16);
  const poleM = new Float32Array(16),
    flagM = new Float32Array(FLAG_SLATS * 16);
  ...
  function spawnSaved(from: number, heap: Heap) {
    ...
    spawnHeap(
      from,
      heap,
      stock.map((n, kind) => (had && n ? Math.min(1, had[kind] / n) : 1)),
    );
  }
  /** What was saved as left of a source, or null if nothing was; a save from before the gold bars has one kind fewer. */
  const hadOf = (a: number) =>
    saved[a]?.length >= 5 ? Array.from({ length: KINDS }, (_, k) => saved[a][k] ?? 0) : null;
  ...
  // So are the hidden chambers broken into off them, and their side rooms, from what was left of each.
  {
    ...
    WALLS.forEach((wall, w) => {
      if (economy.save.walls[w] && wall.treasure.length && inPlay.includes(wall.area) && hadOf(wallSource(w)))
  ...
  function showProgress() {
    ...
    const text = economy.save.done
      ? 'the cave is cleared'
      : `${AREAS[room].name}: ${Math.floor(banked(room) * 100)}% banked` +
        (economy.nextOpen()
          ? ` · on to ${the(economy.next()!)}`
  ...
  function collect(kind: number, x: number, y: number, i: number) {
    ...
    else sound.clink(runCount);
    const gold: [number, number, number] =
      kind === 0
        ? [1.6, 1.2, 0.4]
        : ((dynamic[groupOf(kind)].albedo as [number, number, number]).map((c) => c * 2) as [number, number, number]);
    renderer.emit({
      position: [x, y, 0.5],
      velocity: [0, 0, 12 + heat * 10],
      spread: 6 + heat * 6,
      count: (kind > 0 ? 40 : 8) + Math.round(heat * 30),
      life: 0.9 + heat * 0.5,
      lifeSpread: 0.4,
      size: (kind > 0 ? 0.45 : 0.3) + heat * 0.15,
      growth: -0.2,
      colour: gold,
      alpha: 0,
      gravity: 0.8,
  ...
  function smashOpen(k: number) {
    ...
    for (let i = 0; i < cave.cells.length; i++) {
      ...
      const tx = i % COLS,
        ty = (i / COLS) | 0,
        [w0x, w0y, w1x, w1y] = SECRETS[k].wall;
  ...
  function knockOver(w: number) {
    ...
    layBricks(w).forEach((b, n) => {
      ...
      const vx = c * push + (Math.random() - 0.5) * 3,
        vy = sn * push + (Math.random() - 0.5) * 3,
        vz = 2 + Math.random() * 5;
    ...
    for (const [x, y] of wallTiles(w)) {
      renderer.emit({
        ...
        size: 1.5,
        growth: 1.8,
        colour: WALL_COLOUR[grade].slice(0, 3).map((v) => v * 0.6 + 0.2) as [number, number, number],
        alpha: 0.6,
        gravity: 0.1,
        floor: 0,
      });
    ...
    economy.persist();
    reshape();
    const behind = STASHES.find(
      (st) =>
        st.area === WALLS[w].area &&
        Math.hypot(
          ...(() => {
            const [x0, y0, x1, y1] = WALLS[w].tiles;
            return [(x0 + x1) / 2 - st.at[0], (y0 + y1) / 2 - st.at[1]];
          })(),
        ) < 12,
    );
    note(
      `${WALL_NAME[grade]} wall down${WALLS[w].treasure.length ? ' · something glints in the rubble' : behind ? ` · ${behind.name}` : ''}`,
      3,
    );
  }
  ...
  economy.onChange((id) => {
    ...
    if (id.startsWith('area')) {
      ...
      STASHES.forEach((stash, k) => {
        if (stash.area === a) spawnHeap(stashSource(k), stashHeap(k));
      });
      ...
      for (let i = 0; i < world.count; i++) {
        ...
        if (puffs++ < 160)
          renderer.emit({
            position: [world.x[i], world.y[i], world.z[i] + 0.3],
            velocity: [0, 0, 2],
            spread: 1.5,
            count: 3,
            life: 0.8,
  ...
  dozer.onRock = (tx, ty, square) => {
    ...
    const speed = Math.abs(dozer.speed),
      hard = square >= SMASH_SQUARE && speed >= SMASH_SPEED;
    if (cell >= BRICK) {
      const w = cell - BRICK,
        wall = WALLS[w];
      ...
      renderer.emit({
        position: [x, y, 2],
        velocity: [-Math.cos(dozer.yaw) * 4, -Math.sin(dozer.yaw) * 4, 4],
        spread: 5,
        ...
        size: 0.35,
        growth: -0.2,
        colour: WALL_COLOUR[wall.grade].slice(0, 3) as [number, number, number],
        alpha: 1,
        gravity: 1.4,
        floor: 0,
      });
    ...
    else if (square > 0.2 && speed > 1.5 && t - knockedAt[k] > 0.6) {
      knockedAt[k] = t;
      sound.knock();
    }
  };
  ...
  try {
    const m = localStorage.getItem('pushminer-camera');
    if (MODES.includes(m as CameraMode)) cameraMode = m as CameraMode;
  } catch {
    /* fine */
  }
  ...
  canvas.addEventListener('pointerdown', () => {
    manualUntil = performance.now() / 1000 + 5;
  });
  const wrap = (a: number) => {
  ...
  function cycleCamera() {
    ...
    try {
      localStorage.setItem('pushminer-camera', cameraMode);
    } catch {
      /* fine */
    }
    manualUntil = 0;
    if (cameraMode === 'fixed')
      orbit.setSpherical({ azimuth: orbit.currentAzimuth + wrap(CAMERA.azimuth - orbit.currentAzimuth) });
  ...
  function lightUp(t: number): number[] {
    ...
    lights.add({
      position: [dozer.x - c * 0.8, dozer.y - s * 0.8, 6.5],
      radius: 7,
      colour: [1.0, 0.9, 0.75],
      intensity: 1.2,
    });
    ...
    const [ex, ey] = cam.target,
      lens = 1 / Math.tan((cam.fov * Math.PI) / 360);
    lampsLit.length = 0;
    cave.lamps.forEach((l, k) => {
      if (!lampOn(k)) return;
      ...
      const up = q ? (LAMP_REACH * lens * 1.1) / q[2] : 0,
        across = up / cam.aspect;
      if (q ? Math.abs(q[0]) < 1 + across && Math.abs(q[1]) < 1 + up : Math.hypot(l.x - ex, l.y - ey) < LAMP_REACH)
        lampsLit.push(k);
    });
    lampsLit.sort(
      (a, b) =>
        Math.hypot(cave.lamps[a].x - ex, cave.lamps[a].y - ey) - Math.hypot(cave.lamps[b].x - ex, cave.lamps[b].y - ey),
    );
    ...
    if (p) {
      ...
      quads.set([p[0], p[1], size * 0.9, holePulse * 0.9, 0.5, 1.0, 0.35, 1.8], n * EFFECT_STRIDE);
      n++;
    }
    for (const f of fountains) {
      if (f.glow <= 0) continue;
      ...
      quads.set(
        [
          ...
          f.glow * (f.state === 'warn' ? 0.5 + 0.3 * Math.sin(t * 30) : 1.2),
          1.0,
          0.5,
          0.15,
          2.2,
    ...
    if (warning && n < EFFECT_CAPACITY) {
      ...
      if (q) {
        quads.set([q[0], q[1], 14 / q[2], 0.5 + 0.3 * Math.sin(t * 8), 1.0, 0.25, 0.1, 1.6], n * EFFECT_STRIDE);
        n++;
      }
    }
    ...
    for (const k of lampsLit) {
      ...
      quads.set([q[0], q[1], 5 / q[2], 0.85 + 0.1 * Math.sin(t * 13 + k * 7), 1.0, 0.75, 0.4, 2.0], n * EFFECT_STRIDE);
      n++;
    }
    for (const [x, y] of HOLE_LAMPS) {
      if (n >= EFFECT_CAPACITY - 1) break;
      ...
      if (!q || Math.abs(q[0]) > 1.2 || Math.abs(q[1]) > 1.2) continue;
      quads.set([q[0], q[1], 5 / q[2], 0.9, 0.7, 1.0, 0.6, 2.0], n * EFFECT_STRIDE);
      n++;
    }
  ...
  function upload(t: number) {
    ...
    renderer.move(HULL, hullM);
    renderer.move(DARK, darkM);
    renderer.move(BLADE, bladeM);
  ...
  const showHorn = () => {
    hornButton.hidden = !(touch && economy.save.horn);
  };
  const showMute = () => {
    muteButton.textContent = sound.muted ? '🔇' : '🔊';
    muteButton.setAttribute('aria-label', sound.muted ? 'unmute' : 'mute');
  };
  economy.onChange((id) => {
    if (id === 'horn') showHorn();
  });
  if (touch) {
    ...
    const showControls = () => {
      controlsButton.textContent = controls.scheme === 'tracks' ? '⇅⇅' : '⇅⇆';
    };
  ...
  function pointAlong(ux: number, uy: number) {
    ...
    const w = pointerLabel.offsetWidth / 2 + 24,
      h = pointerLabel.offsetHeight / 2 + 22;
  ...
  function showPointer(t: number) {
    ...
    if (p && p[2] > 0) {
      ...
      const o = project(vp, dozer.x, dozer.y, 0),
        ex = project(vp, dozer.x + 1, dozer.y, 0),
        ey = project(vp, dozer.x, dozer.y + 1, 0);
      if (!o || !ex || !ey) {
        pointer.hidden = true;
        return;
      }
      const wx = target.x - dozer.x,
        wy = target.y - dozer.y;
  ...
  const frame = (now: number) => {
    ...
    if (input.takeShop()) {
      ...
      shopButton.textContent = shopOpen ? 'close' : 'shop';
      if (shopOpen) {
        renderShop(shopRows, economy);
        renderShop(shopCosmetics, economy, economy.cosmetics());
      }
    }
    ...
    for (const b of bots) {
      ...
      if (Math.abs(b.dozer.speed) > 0.5)
        world.wakeNear(
          b.x + bc * BLADE_AT * BOT_SCALE,
          b.y + bs * BLADE_AT * BOT_SCALE,
    ...
    for (const f of fountains) {
      ...
      if (Math.random() < (f.state === 'spray' ? 0.9 : 0.35)) {
        renderer.emit({
          position: [f.x, f.y, 0.3],
          velocity: [0, 0, f.state === 'spray' ? 9 : 2],
          spread: 3,
          count: 4,
          life: 1.2,
          lifeSpread: 0.5,
    ...
    lead[0] += (wantLead[0] - lead[0]) * kl;
    lead[1] += (wantLead[1] - lead[1]) * kl;
    ...
    follow[0] += (aim[0] - follow[0]) * k;
    follow[1] += (aim[1] - follow[1]) * k;
    ...
    renderer.setSunShadow({
      min: [follow[0] - reach, follow[1] - reach, -16],
      max: [follow[0] + reach, follow[1] + reach, 14],
    });
    ...
    if (economy.bank !== lastBank) {
      ...
      bankValue.textContent = shopBalance.textContent = `${economy.bank}`;
      showRun();
    ...
    if (economy.nextOpen()) {
      ...
      if (pastGate(next, dozer.x, dozer.y)) {
        economy.moveOn();
        showProgress();
