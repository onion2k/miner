...
export class Sound {
  ...
  constructor() {
    ...
    try { this.muted = localStorage.getItem('pushminer-muted') === '1'; } catch { /* fine */ }
  }
  ...
  toggleMute(): boolean {
    ...
    try { localStorage.setItem('pushminer-muted', this.muted ? '1' : '0'); } catch { /* fine */ }
    return this.muted;
  }
  ...
  clink(run: number) {
    ...
    for (const [ratio, vol, decay] of [[1, 0.22, 0.14], [2.76, 0.07, 0.07], [5.4, 0.03, 0.05]] as const) {
      const osc = ctx.createOscillator(), gain = ctx.createGain();
      osc.type = 'sine'; osc.frequency.value = f * ratio;
  ...
  thunk(value: number) {
    ...
    if (value >= 25) {
      for (let k = 0; k < 3; k++) {
        ...
        o.type = 'sine'; o.frequency.value = base * (4 + k * 1.5);
  ...
  private startEngine() {
    ...
    osc.frequency.value = 45; osc2.frequency.value = 45.5;
    const filter = ctx.createBiquadFilter(); filter.type = 'lowpass'; filter.frequency.value = 220; filter.Q.value = 2;
  ...
  drive(throttle: number, speed: number, load: number) {
    const e = this.engine, ctx = this.ctx; if (!e || !ctx) return;
    const now = ctx.currentTime;
